...
: name(std::move(nm)), gender(std::move(g)), age(age), weightKg(w), heightCm(h), inclusive(incl), log_(nullptr)
{
    log_ = new CalendarHealth("calendar.txt");
}
...
{
    if (!log_) log_ = new CalendarHealth("calendar.txt");
    ...
    for (int i = 0; i < nFood; ++i)
    {
        ...
        std::string dish; std::getline(std::cin, dish);
        ...
        Meal found = Meal::find_dish(dish);
        nutritions nut(found, grams);

        const double kcal = nut.get_calories();
        const double p    = nut.get_protein();
        const double f    = nut.get_fat();
        const double c    = nut.get_carbs();

        if (!log_->addFood(today, dish, grams, kcal, p, f, c, waterL))
    ...
    for (int i = 0; i < nAct; ++i)
    {
        ...
        std::string actName; std::getline(std::cin, actName);
        ...
        activity a(info, durMin, w);

        if (!log_->addActivity(today, info.name, durMin, w))
            std::cout << "write error (activity)\n";
    }

    if (!log_->writeCaloriesSummary(today)) {
    ...
    if (log_->listByDate(qdate, lines)) {
        std::cout << "\n=== All on " << qdate << " ===\n";
